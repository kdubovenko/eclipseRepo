...
//KD PsychoBass
#include "matlab/PsychoBass.h"
#include "matlab/PsychoBass_initialize.h"

// Variable Definitions
static PsychoBassStackData PsychoBassStackDataGlobal;
static PsychoBassPersistentData PsychoBassPersistentDataGlobal;
...
void processaudio_setup(void) {
	...
	PsychoBassStackDataGlobal.pd = &PsychoBassPersistentDataGlobal;

	// Initialize the application.
	// You do not need to do this more than one time.
	PsychoBass_initialize(&PsychoBassStackDataGlobal);
}
...
#ifdef USE_PSYCHO_BASS
	double freqCutoff = 150;
	double Gain = 0.001;
	double temp_stereo_in[2 * AUDIO_BLOCK_SIZE];
	double temp_stereo_out[2 * AUDIO_BLOCK_SIZE];
	int idx0;
	int idx1;


	//store audio to temp buffer
	for(idx0 = 0; idx0 < 32; idx0++)
	{
		temp_stereo_in[idx0] = (double)audiochannel_0_left_in[idx0];
		temp_stereo_in[idx0+32] = (double)audiochannel_0_right_in[idx0];
	}


	//KD psychoBass code
	PsychoBass(&PsychoBassStackDataGlobal, temp_stereo_in, freqCutoff, Gain, temp_stereo_out);
#endif
...
#ifdef USE_PSYCHO_BASS
    	//KD psychoBass
    	audiochannel_0_left_out[i] = (float)temp_stereo_out[i];
    	audiochannel_0_right_out[i] = (float)temp_stereo_out[i+32];
#else
